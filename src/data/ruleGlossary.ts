...
export const ruleGlossaryEntries: RuleGlossaryEntry[] = [
  {
    ...
    detail: 'A cooling-measure tax layered on top of BSD. Simplified rates used here: SC 0% first home / 20% second home, PR 5% first home / 30% second home, foreigner 60% residential. Some runs can pay ABSD now and recover it later only after a qualifying sale.',
    whyItMatters: 'ABSD is paid with upfront cash first, so it can block a purchase even when your salary and loan size look comfortable. The refund-style paths also teach that policy relief often arrives only after you complete the full replacement move.',
    example: 'A PR buying a second residential property faces 30% ABSD, while a married SC replacement run may still need to front the ABSD cash and recover it only after the first home is sold in time.',
  },
  {
    id: 'bsd',
    label: 'BSD',
  ...
  },
  {
    ...
    detail: 'CPF Ordinary Account balances can offset eligible residential upfront costs. The game applies it before checking how much cash you still need, but short leases can reduce or block how much CPF OA is usable.',
    whyItMatters: 'CPF OA can make a first home reachable sooner, but it does not replace the need for cash buffers, and older leasehold stock can force more of the purchase back into cash.',
    example: 'Using CPF OA can reduce the cash needed for an eligible HDB or condo purchase, while a short-lease unit may allow only reduced CPF support.',
  },
  {
    id: 'mop',
    label: 'MOP',
