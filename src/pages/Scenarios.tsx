...
export default function Scenarios() {
  const { currentScenario, setCurrentScenario, player, addCash, updateCreditScore, applyPropertyValueImpact } = useGameStore();
  ...
  const handleOption = useCallback((option: ScenarioOption) => {
    const roll = Math.random();
    const success = roll <= option.probability;

    if (success) {
      addCash(option.cashImpact);
      updateCreditScore(option.creditImpact);
      if (option.propertyValueImpact !== 0) {
        applyPropertyValueImpact(option.propertyValueImpact);
      }
      setResult(option.followUpText);
    } else {
      addCash(Math.round(option.cashImpact * 0.5));
      updateCreditScore(-10);
      if (option.propertyValueImpact !== 0) {
        applyPropertyValueImpact(Math.round(option.propertyValueImpact * 0.5));
      }
      setResult('Things did not go as planned. The outcome was worse than expected.');
    }
    setResolved(true);
    setCurrentScenario(null);
  }, [addCash, updateCreditScore, applyPropertyValueImpact, setCurrentScenario]);
  ...
  // If there's no active scenario, show scenario list
        ...
        <AnimatePresence>
          ...
          >
            <GlassCard accentColor={color} className="relative">
              <button onClick={handleDismiss} className="absolute top-4 right-4 text-text-dim hover:text-white transition-colors">
                <X size={20} />
              </button>
